def int_dec_to_bin(digit, accuracy=0):
    ...
    def set_format(binary_number, digit_count):
        ...
        """
        assert isinstance(binary_number, str)
        assert isinstance(digit_count, int)
        formatted_number = ""
...
def split_on_bit(permission_number, bits_count=1):
    ...
    :return: list - list, where each digit of number represented in binary form
    """
    assert isinstance(permission_number, str)
    result = []
    number = list(permission_number)
    for num in number:
        # each integer number represent like binary number
        result.append(int_dec_to_bin(int(num), bits_count))
    return result
